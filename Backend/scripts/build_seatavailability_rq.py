...
def build_seatavailability_request(
    ...
    # Extract PricedFlightOffers - for seat availability, we always use the first (and typically only) offer
    ...
    # For seat availability, we use the first offer (flight price responses contain only one offer)
    selected_offer = priced_flight_offers[0]
    logger.info(f"Using first offer for seat availability (index 0 of {len(priced_flight_offers)} total offers)")
