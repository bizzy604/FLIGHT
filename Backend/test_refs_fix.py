#!/usr/bin/env python3
"""
Test the fix for OrderCreate refs issue to ensure dynamic service references
are used instead of hardcoded values like 'SRV13'.
"""
...
def load_test_data():
    """Load test data to verify the fix."""
    try:
        # Load ServiceListRS with services that have ObjectKeys
...
def test_refs_fix():
    """Test that the refs fix works correctly."""
    print("=" * 80)
    print("TESTING REFS FIX")
    print("=" * 80)
    ...
    # Find services with ObjectKeys (not hardcoded IDs)
    ...
    try:
        ...
        for i, item in enumerate(order_items):
            ...
            for j, other_item in enumerate(other_items):
                ...
                # Check if refs contain the service ObjectKey (dynamic)
                if first_service_key in refs:
                    print(f"    ✅ Contains dynamic service ObjectKey: {first_service_key}")
                else:
...
def main():
    """Main test function."""
    print("🔍 TESTING ORDERCREATE REFS FIX")
    print("=" * 80)
    
    success = test_refs_fix()
    ...
    if success:
        print("✅ REFS FIX TEST PASSED")
        print("  - Dynamic service ObjectKeys are used")
        print("  - No hardcoded service IDs like 'SRV13'")
        print("  - OrderCreate payload is correctly generated")
    else:
        print("❌ REFS FIX TEST FAILED")
